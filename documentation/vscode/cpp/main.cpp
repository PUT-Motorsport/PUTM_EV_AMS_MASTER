...
int main()
{
    logger.log_error("A", 0);
    for(size_t i = 1; i < 128; i++) logger.log_error("B", i);
    // std::cout << logger.get_buffer();
    // for(size_t i = 0; i < logger.message_count(); i++)
    // {
    //     auto message = logger.get_message(i);
    //     std::cout << message << std::endl;
    // }
    for(auto message : logger)
    {
        std::cout << message << std::endl;
    }
