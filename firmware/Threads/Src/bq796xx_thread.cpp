...
VOID bq796xx_thread_entry(__unused ULONG thread_input)
{
    ...
    HAL_StatusTypeDef read_status_status { HAL_OK };
    HAL_StatusTypeDef read_data_status { HAL_OK };

    data.bq_init_status = bq.init();
    bq.read_stack_data(data.cell_voltages, data.gpio_voltages);
    bq.read_stack_data(data.cell_voltages, data.gpio_voltages);
    ...
    while(true)
    {
        ...
        // {      
        //     data.bq_read_status_status = bq.read_stack_status(data.cell_ovuv,
        //                                                       data.cell_otut);
        ...
        // {
        //     /* data.bq_read_data_status[device_address - 1] = */
        ...
        //                                          data.cell_max_temperature);
        //     data.cell_min_temperature = std::min(*std::min_element(std::begin(data.cell_temperatures[device_address - 1]), 
        //                                                            std::end(data.cell_temperatures[device_address - 1])),
