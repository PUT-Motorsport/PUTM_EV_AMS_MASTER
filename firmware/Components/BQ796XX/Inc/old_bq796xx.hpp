...
namespace PUTM
{
    namespace Bq796xx
    {
        ...
        public:
             ...
             *  @param  `ovuv` pair of undervoltage and overvoltage thresholds in mV, must be between 1200 and 3100 for undervoltage 
             *          and between 4175 and 4475 for overvoltage
             *  @param  `otut` pair of undertemperature and overtemperature thresholds
             *          undertemperature threshold represendeted as a percentage 66% to 80% in steps of 2!
             *          overtemperature threshold represendeted as a percentage 10% to 39% in steps of 1
             *  @retval	HAL_OK when done, HAL_BUSY when init in progress, HAL_ERROR on fail
             */
            HAL_StatusTypeDef init(std::pair<uint32_t, uint32_t> ovuv, std::pair<uint32_t, uint32_t> otut)
        public:
            /**
        ...
        public:
            ...
            *	@brief Poll stack status to local storage
             *  @return HAL_OK
            */
            HAL_StatusTypeDef read_stack_status(bool (&ovuv_arr)[Config::TOTAL_CELL_COUNT], 
                                                bool (&otut_arr)[Config::TOTAL_TEMPERATURE_COUNT]);
        public:
            /**
        ...
        public:
            /**
             *  @brief 	Read single data from a device at `address`
             *  @param  voltages_arr array to copy voltages to, size must be equal to Config::CELL_COUNT_PER_DEVICE
             *  @param  temperatures_arr array to copy temperatures to, size must be equal to Config::TEMPERATURES_COUNT_PER_DEVICE
             *  @param  address address of a device to be read from, must be in range [1, Config::STACK_SIZE].
             *          Devices are addressed from 1 to Config::STACK_SIZE starting from the first device connected to the master
             *          at High Side Communication port.
             *  @retval	HAL_OK when done, HAL_BUSY when read in progress, HAL_ERROR on fail
             *  @note   This function is blocking, it will wait for the data to be received or timeout after Config::STACK_COM_TIMEOUT
             */
            HAL_StatusTypeDef read_signle_data(float (&voltages_arr)[Config::CELL_COUNT_PER_DEVICE], 
        ...
        public:
            ...
            */
            ...
            HAL_StatusTypeDef write_single(uint8_t (&data)[DATA_COUNT], uint16_t reg_address, uint8_t dev_address = 1)
            {
                return write<ReqType::Single>(data[0], DATA_COUNT, reg_address, dev_address);
            }
            template<size_t DATA_COUNT>
            HAL_StatusTypeDef write_stack(uint8_t (&data)[Config::STACK_SIZE][DATA_COUNT], uint16_t reg_address)
            {
                return write<ReqType::Stack>(data[0], DATA_COUNT, reg_address, dev_address);
            }
            template<size_t DATA_COUNT>
            HAL_StatusTypeDef write_broadcast(uint8_t (&data)[Config::STACK_SIZE + 1][DATA_COUNT], uint16_t reg_address)
            {
                return write<ReqType::Broadcast>(data[0], DATA_COUNT, reg_address, dev_address);
            }
        public:
            ...
            */
            ...
            template<size_t DATA_COUNT>
            HAL_StatusTypeDef read_single(uint8_t (&data)[DATA_COUNT], uint16_t reg_address, uint8_t dev_address = 1)
            {
                return read<ReqType::Single>(data[0], DATA_COUNT, reg_address, dev_address);
            }
            template<size_t DATA_COUNT>
            HAL_StatusTypeDef read_stack(uint8_t (&data)[Config::STACK_SIZE][DATA_COUNT], uint16_t reg_address)
            {
                return read<ReqType::Stack>(data[0], DATA_COUNT, reg_address, dev_address);
            }
            template<size_t DATA_COUNT>
            HAL_StatusTypeDef read_broadcast(uint8_t (&data)[Config::STACK_SIZE + 1][DATA_COUNT], uint16_t reg_address)
            {
                return read<ReqType::Broadcast>(data[0], DATA_COUNT, reg_address, dev_address);
            }
        };   
    }
}