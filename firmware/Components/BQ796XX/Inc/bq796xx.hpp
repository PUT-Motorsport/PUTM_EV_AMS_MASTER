    ...
    namespace Bq796xx
    {
        ...
        private:
            ...
            HAL_StatusTypeDef write(uint8_t (&data)[DATA_COUNT], uint16_t reg_address, uint8_t dev_address = 1)
            {
                ...
                if constexpr(COMMUNICATION_TYPE == CommunicationMode::Single) tx_buffer[i++] = dev_address;
        ...
        private:
            ...
            HAL_StatusTypeDef read(uint8_t (&data)[DEVICE_COUNT][DATA_COUNT], uint16_t reg_address, uint8_t dev_address = 1)
            {
                ...
                if(uart.await_tx_rx_dma(tx_buffer, i, (uint8_t*)data, DEVICE_COUNT * DATA_COUNT + 2) != HAL_OK) return HAL_ERROR;
